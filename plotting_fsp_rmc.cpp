...
void plotting_fsp_rmc(){
    ...
    TFile* readfile_MC = SafelyOpenRootfile("/Users/lauragans-bartl/master/data/AnalysisResults_LHC25d2_1217.root"); 
    // TFile* readfile_MC = SafelyOpenRootfile("/Users/lauragans-bartl/Downloads/AnalysisResults_allConfigs.root");  
    if (!readfile_MC || readfile_MC->IsZombie()) {
        std::cerr << "MC file konnte nicht geöffnet werden!\n";
    }

    std::unique_ptr<THnSparseD> hmpt_same_rMC(readfile_MC->Get<THnSparseD>("pi0eta-to-gammagamma-pcmdalitzee/Pair/same/hs"));
    std::unique_ptr<THnSparseD> hmpt_mix_rMC(readfile_MC->Get<THnSparseD>("pi0eta-to-gammagamma-pcmdalitzee/Pair/mix/hs"));
    std::unique_ptr<TH1D> hNCollision_rMC(readfile_MC->Get<TH1D>("pi0eta-to-gammagamma-pcmdalitzee/Event/after/hCollisionCounter"));
    ...
    //Massepeak in Pt
    std::unique_ptr<TH1D>  h_MmaxPTEta = std::make_unique<TH1D>(" h_MmaxPTEta",";#it{p}_{T} (GeV/#it{c}); #LT m_{#eta} #GT (GeV/#it{c}^{2})", vec_pt_min.size()-1, vec_pt_min.data()); 
    SetHistoStandardSettings1D( h_MmaxPTEta.get(), 1.2, 1.6); 

    std::unique_ptr<TH1D>  h_MmaxPTPion = std::make_unique<TH1D>(" h_MmaxPTPion",";#it{p}_{T} (GeV/#it{c}); #LT m_{#pi_{0}} #GT (GeV/#it{c}^{2})", vec_pt_min.size()-1, vec_pt_min.data()); 
    ...
    for(int proj = 0; proj < 2; proj++){             // 0 = minv - histo, 1 = pt - histo
        TH1D* h_same = hmpt_same_rMC->Projection(proj);
        h_same->SetName(Form("h_name_same_%i", proj));
        SetHistoStandardSettings1D(h_same);
        h_same->Draw("pe");
        vec_hProject_same_zozp.push_back(h_same);
        // canvas->SaveAs(Form("Plots_25d2_rmc/hproj_same_%s.png", vec_proj_type[proj].data()));
        TH1D* h_mix= hmpt_mix_rMC->Projection(proj); 
        h_mix->SetName(Form("h_name_mix_%i", proj));
        SetHistoStandardSettings1D(h_mix);
        h_mix->Draw("pe");
        vec_hProject_mix_zozp.push_back(h_mix);
       
        // canvas->SaveAs(Form("Plots_25d2_rmc/hproj_mix_%s.png", vec_proj_type[proj].data()));
    }   
    ...
    for(int i = 0; i < vec_pt_min_bin_same.size()-1 ; i++){                    //Projections of specific pt-ranges
        //same
        THnSparseD* hmpt_same_clone =  (THnSparseD*) hmpt_same_rMC->Clone(Form("hmpt_same_clone_%i", i));
        ...
        // canvas->SaveAs(Form("Plots_25d2_rmc/hproj_same_%0.1f-%0.1f_Rebin%i.png", vec_pt_min[i], vec_pt_min[i+1], NRebin));

        //mixed
        THnSparseD* hmpt_mix_clone =  (THnSparseD*) hmpt_mix_rMC->Clone(Form("hmpt_mix_clone%i", i));
        ...
        // canvas->SaveAs(Form("Plots_25d2_rmc/hproj_mix_%0.1f-%0.1f_Rebin%i.png", vec_pt_min[i], vec_pt_min[i+1], NRebin));
    ...
    for(int i = 0; i < vec_h_same_projm_ptranges_zozp.size(); i++){
        ...
        // canvas->SaveAs(Form("Plots_25d2_rmc/h_mix_scaled_%0.1f-%0.1f.png",vec_pt_min[i], vec_pt_min[i+1]));

        TH1* hSameScaledMixRatio = MakingRatio(h_same, h_mix_scaled, "hSameScaledMixRatio");
        hSameScaledMixRatio->Draw("");
        canvas->SaveAs(Form("Plots_25d2_rmc/hSameScaledMixRatio%0.1f-%0.1f.png",vec_pt_min[i], vec_pt_min[i+1]));

        // TH1* hSameMixRatio = MakingRatio(h_same, h_mix, "hSameMixRatio");
        // hSameMixRatio->Draw("");
        // canvas->SaveAs(Form("Plots_25d2_rmc/hSameMixRatio_%0.1f-%0.1f.png",vec_pt_min[i], vec_pt_min[i+1]));
        ...
        h_signal->GetYaxis()->SetRangeUser(-100., 1300.);
        h_signal->Draw("pe");
        ...
        // DrawingHeaderStandardLinesMC(0.4, 0.70, 0.65, 0.60);
        ...
        canvas->SaveAs(Form("Plots_25d2_rmc/h_signal_eta_pion%0.1f-%0.1f.png", vec_pt_min[i], vec_pt_min[i+1]));
        ...
        canvas->SaveAs(Form("Plots_25d2_rmc/hSignalPion_%0.1f_%0.1f.png", vec_pt_min[i], vec_pt_min[i+1]));
        ...
        //Legende fuer 0.8< pt < 0.9
        DrawingHeaderStandardLinesMC(0.13, 0.91, 0.28, 0.23);
        ...
        canvas->SaveAs(Form("Plots_25d2_rmc/hSignalEta_%0.1f_%0.1f.png", vec_pt_min[i], vec_pt_min[i+1])); 
    ...
    // canvas->SaveAs("Plots_25d2_rmc/h_EtaSoverB.png");
    ...
    // canvas->SaveAs("Plots_25d2_rmc/h_PionSoverB.png");
    ...
    DrawingHeaderStandardLinesMC(0.4, 0.50, 0.45, 0.40);
    ...
    canvas->SaveAs("Plots_25d2_rmc/h_PionEtaSoverB.png");
