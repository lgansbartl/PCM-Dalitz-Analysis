...
void plotting_fsp_data(){
    ...
    //open file
    TFile* readfile_zozp = SafelyOpenRootfile("/Users/lauragans-bartl/master/data/AnalysisResults_LHC23zozp.root");  
    ...
    for(int proj = 0; proj < 2; proj++){             // 0 = minv - histo, 1 = pt - histo
        ...
        // canvas->SaveAs(Form("Plots_zozp_fsp/hproj_same_%s.png", vec_proj_type[proj].data()));
        ...
        // canvas->SaveAs(Form("Plots_zozp_fsp/hproj_mix_%s.png", vec_proj_type[proj].data()));
    }   
    ...
    for(int i = 0; i < vec_pt_min_bin_same.size()-1 ; i++){                    //Projections of specific pt-ranges
        ...
        // canvas->SaveAs(Form("Plots_zozp_fsp/hproj_same_%0.1f-%0.1f_Rebin%i.png", vec_pt_min[i], vec_pt_min[i+1], NRebin));
        ...
        // canvas->SaveAs(Form("Plots_zozp_fsp/hproj_mix_%0.1f-%0.1f_Rebin%i.png", vec_pt_min[i], vec_pt_min[i+1], NRebin));
    ...
    for(int i = 0; i < vec_h_same_projm_ptranges_zozp.size(); i++){
        ...
        // canvas->SaveAs(Form("Plots_zozp_fsp/h_mix_scaled_%0.1f-%0.1f.png",vec_pt_min[i], vec_pt_min[i+1]));

        TH1* hSameScaledMixRatio = MakingRatio(h_same, h_mix_scaled, "hSameScaledMixRatio");
        hSameScaledMixRatio->Draw("");
        canvas->SaveAs(Form("Plots_zozp_fsp/hSameScaledMixRatio%0.1f-%0.1f.png",vec_pt_min[i], vec_pt_min[i+1]));

        // TH1* hSameMixRatio = MakingRatio(h_same, h_mix, "hSameMixRatio");
        // hSameMixRatio->Draw("");
        // canvas->SaveAs(Form("Plots_zozp_fsp/hSameMixRatio_%0.1f-%0.1f.png",vec_pt_min[i], vec_pt_min[i+1]));
        ...
        canvas->SaveAs(Form("Plots_zozp_fsp/h_signal_eta_pion%0.1f-%0.1f.png", vec_pt_min[i], vec_pt_min[i+1]));
        ...
        canvas->SaveAs(Form("Plots_zozp_fsp/hSignalPion_%0.1f_%0.1f.png", vec_pt_min[i], vec_pt_min[i+1]));
        ...
        canvas->SaveAs(Form("Plots_zozp_fsp/hSignalEta_%0.1f_%0.1f.png", vec_pt_min[i], vec_pt_min[i+1])); 
    ...
    // canvas->SaveAs("Plots_zozp_fsp/h_EtaSoverB.png");
    ...
    // canvas->SaveAs("Plots_zozp_fsp/h_PionSoverB.png");
    ...
    canvas->SaveAs("Plots_zozp_fsp/h_PionEtaSoverB.png");
    ...
    canvas->SaveAs("Plots_zozp_fsp/h_MmaxPTEta.png");
    ...
    canvas->SaveAs("Plots_zozp_fsp/h_MmaxPTPion.png");
